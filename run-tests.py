...
def run_reader_tests(root_dir):
    print("##### Running Delta Reader tests #####")
    reader_root_dir = path.join(root_dir, path.join("extras", "reader"))
    with WorkingDirectory(reader_root_dir):
        sbt_path = path.join(".", path.join("build", "sbt"))
        run_cmd([sbt_path, "clean", "test"], stream_output=True)
