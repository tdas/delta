...
import org.apache.spark.sql.delta.{DeltaLog, DeltaLogReader, SnapshotLike}
...
trait DeltaLogging
  ...
  protected def recordDeltaEvent(
    ...
    recordDeltaEvent(
      Try(deltaLog.dataPath).getOrElse(null),
      Try(deltaLog.snapshot.metadata.id).getOrElse(null),
      opType,
      tags,
      data)
  }
