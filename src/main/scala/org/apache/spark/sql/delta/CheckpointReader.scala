...
trait CheckpointReader extends DatabricksLogging with Logging {
  ...
  private def loadMetadataFromFile(tries: Int): Option[CheckpointMetaData] = {
    try {
      val checkpointMetadataJson = read(LAST_CHECKPOINT)
  ...
  protected def findLastCompleteCheckpoint(cv: CheckpointInstance): Option[CheckpointInstance] = {
    ...
    while (cur >= 0) {
      val checkpoints = listFrom(checkpointPrefix(logPath, math.max(0, cur - 1000)))
        .map(_.getPath)
