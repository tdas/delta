 ...
 */
object DeltaConfigs extends DatabricksLogging {
    ...
    configurations.map {
      case (key, value) if key.toLowerCase(Locale.ROOT).startsWith("delta.") =>
        Option(entries.get(key.toLowerCase(Locale.ROOT).stripPrefix("delta.")))
          .map(_(value))
          .getOrElse {
            throw unknownConfigurationKeyException(key)
          }
      case keyvalue @ (key, _) =>
  ...
  private def unknownConfigurationKeyException(confKey: String): Throwable = {
