...
class Snapshot(
  ...
  extends SnapshotLike[Dataset[SingleAction], Dataset[AddFile]]
  with StateCache
  ...
  override val State(protocol, metadata, setTransactions, sizeInBytes, numOfFiles, numOfMetadata,
...
object Snapshot extends DeltaLogging {
   ...
   */
  def assertLogBelongsToTable(logBasePath: URI): UserDefinedFunction = {
    udf((filePath: String) => {
